...
import de.tuberlin.cit.softmon.gui.VizualizationPanel;
...
import de.tuberlin.cit.softmon.rest.floodlight.model.topo.Device;
...
public class MainController {
	...
	private String[] m_selectedGraphItem;
	...
	public MainController() {
		...
		m_selectedGraphItem = new String[]{"",""};
		addListeners();
	...
	private void addListeners() {
		...
		m_window.getVizualizationPanel().addButtonListener(new VizPanelListener());
		// m_window.getTopologyPanel().addTreeMouseListener(new
		// TopologyTreeMouseListener()); // for future use
	...
	class VizPanelListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			String command = e.getActionCommand();
			int cycleTime = m_window.getGraphOptionsPanel().getCycleTime();

			
			System.out.println(": " + m_selectedGraphItem[0] + " "+m_selectedGraphItem[1]);
			
			
			if (!(m_selectedGraphItem[0].equals("")))

				switch (command) {
				case VizualizationPanel.SHOW_BUTTON_TITLE:
					if (m_selectedGraphItem[1].equals(""))
						if (m_selectedGraphItem[0].contains(":")) {
							OfSwitch ofSwitch = m_topology.getSwitch(m_selectedGraphItem[0]);
						...
						} if (!m_selectedGraphItem[0].equals("") && m_selectedGraphItem[0].contains(":")  && !m_selectedGraphItem[1].equals("")){
							
							
							int portID = Integer.parseInt((m_selectedGraphItem[1].substring(m_selectedGraphItem[1].length() - 1)));
							
							System.out.println("port id : " + portID);
							
							
							OfPort ofPort =  m_topology.getSwitch(m_selectedGraphItem[0]).getPort(portID);
				...
				// if port
				case VizualizationPanel.RESET_BUTTON_TITLE:
					m_selectedGraphItem[0] = m_selectedGraphItem[1] = "";
					m_window.getVizualizationPanel().resetWindow();
					
					System.out.println("nie s ale: " + m_selectedGraphItem[0] + " "+m_selectedGraphItem[1]);
					if (m_dataSource != null)
						m_dataSource.stop();
					m_dataSource = null;
					...
					// selected tree node type isn't necessarily the type of the
	...
	class VizPanelGraphVertexListener implements ItemListener {

		@Override
		public void itemStateChanged(ItemEvent e) {
			Object subject = e.getItem();
			
				m_selectedGraphItem[0] = subject.toString();
		}

	}

	class VizPanelGraphEdgeListener implements ItemListener {

		@Override
		public void itemStateChanged(ItemEvent e) {
			Object subject = e.getItem();
			
			m_selectedGraphItem[1] = subject.toString();
		}
		...
		public void actionPerformed(ActionEvent e) {
			...
			case RestConnectorPanel.APPLY_BUTTON_TITLE:
				...
				if (success) {
					...
					m_window.getVizualizationPanel().refreshTopology(m_topology);
					m_window.getVizualizationPanel().addVertexListener(new VizPanelGraphVertexListener());
					m_window.getVizualizationPanel().addEdgeListener(new VizPanelGraphEdgeListener());
	...
	class TopologyPanelListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			case TopologyPanel.SHOW_BUTTON_TITLE:
				...
				// TopologyPanel.SHOW_BUTTON_TITLE + "\" pressed.");
				int cycleTime = m_window.getGraphOptionsPanel().getCycleTime();
				if (!(m_selectedTreeNodeType == null)) {
					// status message
					if (m_selectedTreeNodeType.equals(TreeUtils.NODE_TYPE_SWITCH)
							|| m_selectedTreeNodeType.equals(TreeUtils.NODE_TYPE_LINK)) {
					...
					case TreeUtils.NODE_TYPE_FLOW:
						...
						if (dataSourceFlowHasChanged) {
							...
							// detect minimum current speed which is different
							// from 0
