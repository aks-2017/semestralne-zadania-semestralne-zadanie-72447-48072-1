...
public class FloodlightRestConnector extends ARestConnector{
	...
	private OfTopology getBasicTopology() {
		...
		Map<String, OfDevice> deviceMap = new TreeMap<>();
		
		try {
			switches = m_client.getSwitches();
			links = m_client.getLinks();
			devices = m_client.getDevices();
		...
		if (!(devices == null)) {
			for (Device dvc : devices) {
				...
				if(!(dvc.attachmentPoint.length == 0)){
					OfDevice OfDvc = new OfDevice(Arrays.toString(dvc.ipv4), dvc.attachmentPoint[0].switchDPID, Integer.toString(dvc.attachmentPoint[0].port));
					deviceMap.put(Arrays.toString(dvc.ipv4), OfDvc);
				}
			}
		} 
		
		
		return new OfTopology(switchMap, linkMap, deviceMap);
	}
...
//							
					...
					//System.out.println("IIIIIIIIIIIIIIIIIIIIIIIIIINNNNNNNNNNNNNNNNNNNNNNNVEAAAAAAAAAAAAAAAAAAA");
				}
			}
		} 
		return flowList;
	...
	public OfPortCounter getOfPortCounter(String dpid, String port) {
		...
		try {
			portReply = m_client.getPortStatistics(dpid.toString());
		} catch (Exception e) {
			System.out.println("Exception: " + e.getMessage());
			e.printStackTrace();
		}
		...
		if (portReply != null) {
			...
			for (Port p : portReply.port) {
				...
				// System.out.println(port + " -- "+ p.portNumber);
